...
class ShopCubit extends Cubit<ShopStates> {
  ...
  List<Widget> screens = [HomeScreen(),FavoriteScreen(), CartScreen(), UserScreen()];
  ...
  Future<void> getProductDetailsModel({
    int? id,
  }) async {
    emit(ShopLoadingDetailsState());
    Dio_Helper.getData(
  ...
  void changCart(int? productId) {
    ...
    Dio_Helper.postData(
      ...
      if (!changeCartsModel!.status!) { //معناها ان الاستيتس بفولس  يعني هيكون المنتج في ال كارت عادي
        carts[productId] != carts[productId];
      } else {  // معناها ان الاستيتس ب ترو يعني المنتج لسه مرحش ال الكارت هدوس هيروح وهيبقي ب تروو
        getCart();
      }
      showToast(
  ...
  Map<int, bool> favorites = {};//ماب بتاخد بولين و اينت لو ال الاي دي فافوريت يبقي  ترو ايلس يبقي فولس
  ...
  void changFavorite(int? productId) {
    ...
    Dio_Helper.postData(
      ...
      if (!updateFavoriteModel!.status!) { //معناها ان الاستيتس بفولس  يعني هيكون المنتج في ال كارت عادي
        favorites[productId] != favorites[productId];
      } else {  // معناها ان الاستيتس ب ترو يعني المنتج لسه مرحش ال الكارت هدوس هيروح وهيبقي ب تروو
        getFavorite();
      }
